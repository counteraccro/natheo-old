    ...
    public function selectTagForEntity(): string
    {
        $html = '<label for="tag-list" class="form-label">' . $this->translator->trans('admin_faq#Tags') . '</label>
                    <div class="input-group mb-3">
                        ...
                        <button class="btn btn-primary" data-url="' . $this->router->generate(('admin_tag_ajax_tmp_add')) . '" data-loading="' . $this->translator->trans('admin_tag#Chargement de la modale pour ajouter un tag') . '" type="button" id="btn-modal-add-tag-for-entity">' . $this->translator->trans('admin_faq#Nouveau tag') . '</button>
